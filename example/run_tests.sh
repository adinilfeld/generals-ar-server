...
if [ "$worker_type" == "local" ]; then
    run_in_terminal "cd '$current_directory'; python3 task_dispatcher.py -m local -p 8888 -w 3"
elif [ "$worker_type" == "pull" ]; then
    run_in_terminal "cd '$current_directory'; python3 task_dispatcher.py -m pull -p 8888"
    sleep 1
    run_in_terminal "cd '$current_directory'; python3 pull_worker.py 3 tcp://127.0.0.1:8888"
elif [ "$worker_type" == "push" ]; then
    run_in_terminal "cd '$current_directory'; python3 task_dispatcher.py -m push -p 8888"
    sleep 1
    run_in_terminal "cd '$current_directory'; python3 push_worker.py 3 tcp://127.0.0.1:8888"
else
    echo "Invalid worker type: $worker_type"
    exit 1
fi
sleep 3 # Wait for the workers to connect

# Terminal 5: Run pytest tests
run_in_terminal "cd '$current_directory'; pytest tests -s"
